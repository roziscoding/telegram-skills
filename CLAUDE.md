...
A bundle of [agent skills](https://skills.sh) that let AI agents interact with users via Telegram. It is installed by end users with `bunx skills add roziscoding/telegram-skills`. The four user-facing skills (`telegram-setup`, `telegram-notify`, `telegram-ask`, `telegram-manage-contacts`) are thin SKILL.md wrappers; all real logic lives in a single CLI. A fifth skill, `telegram-skills-bin`, is infrastructure: it ships the shared CLI wrapper that the others depend on.
...
- `lib/telegram.ts` — the entire CLI. A single grammY-based script that dispatches on flags: `--list-contacts` / `--contact-add` / `--delete-contact` (contact management) and `--setup` (interactive connect flow) exit early, otherwise message-sending mode. Reads the message/question from **stdin**, resolves recipients, sends via `bot.api.sendMessage` with Markdown parse mode.

  `--setup [<token>] [--name <contact>]`: generates an in-memory single-use code, calls `getMe` for the bot username, prints a `https://t.me/<bot>?start=<code>` deeplink block to **stderr** (so the agent can relay it before the process blocks), then long-polls. A deeplink click and a manual `/start <code>` are identical, so one `bot.command("start")` handler covers both; it ignores any payload that isn't the code. On match it stores the chat ID under a contact (the `--name` override, else the sender's first name, else `me`), sets it as `default_contact`, prints `SETUP_OK` to **stdout**, and exits. The token from the positional arg is persisted up front. The skill must run this in the background to read the deeplink before it blocks.
...
- `skills/{telegram-setup,telegram-notify,telegram-ask,telegram-manage-contacts}/SKILL.md` — the four user-facing manifests. They contain no logic; they document how to call `../telegram-skills-bin/bin/telegram` for each use case, and note the dependency on `telegram-skills-bin`.
